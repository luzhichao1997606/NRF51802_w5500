...
/***        Macro Definitions                                               ***/
/******************************************************************************/
uint32 SystemNowtime;//????????10ms
uint32 uip_timer=0;//uip???,?10ms??1.
/******************************************************************************/
/***        Type Definitions                                                ***/
/******************************************************************************/
...
static void halTimInit(void)
{
   ...
   */
//   TIM2_TimeBaseInit(TIM2_PRESCALER_16, 999);  // 1ms????
//   TIM2_PrescalerConfig(TIM2_PRESCALER_16, TIM2_PSCRELOADMODE_IMMEDIATE);
//   TIM2_ARRPreloadConfig(ENABLE);
//   TIM2_ITConfig(TIM2_IT_UPDATE, ENABLE);
//   TIM2_Cmd(ENABLE);
   
   //TIM3_TimeBaseInit(TIM3_PRESCALER_16, 999);  // 1ms????
...
void nrf_timer_delay_ms(uint_fast16_t volatile number_of_ms)
{
    ...
    NRF_TIMER0->BITMODE        = TIMER_BITMODE_BITMODE_16Bit;  // 16 bit ??.
...
* Decription   : ???????????u16Delay ms
* Calls        : 
* Called By    :
* Arguments    : u16Delay: ????(ms)
* Returns      : None
* Others       : ????
******************************* Revision History *******************************
* No.   Version   Date          Modified By   Description               
